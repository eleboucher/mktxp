...
func (c *GRECollector) Collect(ctx context.Context, e *entry.RouterEntry, ch chan<- prometheus.Metric) error {
	...
	for _, raw := range records {
		...
		if rec["running"] == trueStr && rec["disabled"] != trueStr {
			mb.GaugeVal(ch, "gre_status", "GRE tunnel status (1=running, 0=stopped)", 1.0, labelKeysWithRouter, []string{e.RouterID["router_id"], rec["name"], rec["remote_address"]})
		} else {
			mb.GaugeVal(ch, "gre_status", "GRE tunnel status (1=running, 0=stopped)", 0.0, labelKeysWithRouter, []string{e.RouterID["router_id"], rec["name"], rec["remote_address"]})
		}
