...
// HealthCollector collects hardware health metrics (voltage, temperature, fans, PSU).
type HealthCollector struct{}

func NewHealthCollector() *HealthCollector                    { return &HealthCollector{} }
...
func (c *HealthCollector) Collect(ctx context.Context, e *entry.RouterEntry, ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		slog.Error("health collect failed", "router", e.RouterName, "err", err)
