...
func (c *IPIPCollector) Collect(ctx context.Context, e *entry.RouterEntry, ch chan<- prometheus.Metric) error {
	...
	for _, raw := range records {
		...
		if rec["running"] == trueStr && rec["disabled"] != trueStr {
			mb.GaugeVal(ch, "ipip_status", "IPIP tunnel status (1=running, 0=stopped)", 1.0, labelKeysWithRouter, []string{rec["name"], rec["remote_address"]})
		} else {
			mb.GaugeVal(ch, "ipip_status", "IPIP tunnel status (1=running, 0=stopped)", 0.0, labelKeysWithRouter, []string{rec["name"], rec["remote_address"]})
		}
